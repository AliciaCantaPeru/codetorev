...
@Component({
  selector: 'app-usu-bmo-crear-editar',
  templateUrl: './usu-bmo-crear-editar.component.html',
  styleUrls: ['./usu-bmo-crear-editar.component.scss'],
})
export class UsuBmoCrearEditarComponent implements OnInit {
  formGroup: FormGroup;
  listacliente: Opcion[] = [];
  ...
  guardar() {
    ...
    if (this.id == -1) {
      this.usuarioService.guardar(mant).subscribe(
        (res) => {
          this.router.navigate([PathApp.routeCuentaBmoi]);
