 ...
 */
@RestController
@RequestMapping("api/usuario/seller")
public class UsuarioSellerController {

    @Autowired
    private MensajeSupport mensajeSupport;

    @Autowired
    private UsuarioSellerService usuarioService;

    @GetMapping("{ipPersona}/{idPersona}")
    public Respuesta<UsuarioMantDto> obtener(@PathVariable int ipPersona, @PathVariable int idPersona) throws Exception {
        UsuarioMantDto mantDto = usuarioService.obtener(ipPersona, idPersona);
