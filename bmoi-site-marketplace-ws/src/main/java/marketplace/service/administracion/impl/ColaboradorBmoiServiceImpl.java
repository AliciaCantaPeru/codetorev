...
//    private SolicitudesRepository solicitudesRepository;
    ...
    public PageImpl<ColaboradorSellerOutDto> listar(ColaboradorSellerInDto in) throws Exception {
        ...
        PageImpl<SellerPersonas> pageEntity = personasRepository.listarSellerPersonas(TipoUsuarioLogin.BMOI.getValor(), in.getSortCampo(), in.getSortOrden(), in.getTexto(), cuentaEntity.getIdSellerpersona().getIdseller().getId(), pageRequest, false);
    ...
    public ColaboradorSellerMantDto guardar(MultipartFile image, ColaboradorSellerMantDto mantenimientoDto) throws Exception {
        String usuario = cuentasRepository.validaObtenerCorreo(mantenimientoDto.getIdPersonaLogeada(), TipoUsuarioLogin.BMOI.getValor());
        String foto = null;
        if (image != null && !image.isEmpty()) {
            foto = amazonClientService.uploadFile(image, "bmoi-colaboradores/");
        }
